...
const APP_DISPLAY_NAME = isDevelopment ? "Lygos Dev (Dev)" : "Lygos Dev (Alpha)";
const APP_USER_MODEL_ID = "com.lygos.lygosdev";
...
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "T3 Code (Dev)" : "T3 Code (Alpha)"; // keep for migration
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Lygos Dev failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
